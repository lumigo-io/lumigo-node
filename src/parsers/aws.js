...
export class AwsParser {
  ...
  static dynamodbParser = (requestData) => {
    ...
    const resourceName = this.extractDynamodbTableName(reqBodyJSON, dynamodbMethod);
    const messageId = this.extractDynamodbMessageId(reqBodyJSON, dynamodbMethod);
  ...
  static lambdaParser = (requestData, responseData) => {
    ...
    resourceName = this.isArn(resourceName)
      ? this.extractLambdaNameFromArn(resourceName)
      : resourceName;
