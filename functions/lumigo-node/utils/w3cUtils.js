...
const generateMessageId = () => getRandomString(16);
export const shouldSkipTracePropagation = (headers) => {
    return Object.keys(headers).some((key) => SKIP_INJECT_HEADERS.includes(key.toLowerCase()));
};
export const getW3CTracerPropagatorAdditionalHeaders = (headers) => {
...
export const addW3CTracePropagator = (headers) => {
    const additionalHeaders = getW3CTracerPropagatorAdditionalHeaders(headers);
    return Object.assign(headers, additionalHeaders);
};
const parseW3CHeader = (headers) => {
    const existingHeader = (headers === null || headers === void 0 ? void 0 : headers[TRACEPARENT_HEADER_NAME]) || '';
...
const getTraceId = (headers, transactionId, messageID) => {
...
const getTraceState = (headers, messageID) => {
    const lumigoState = `lumigo=${messageID}`;
    if (headers === null || headers === void 0 ? void 0 : headers[TRACESTATE_HEADER_NAME]) {
        return headers[TRACESTATE_HEADER_NAME] + ',' + lumigoState;
    }
    return lumigoState;
};
export const getW3CMessageId = (headers) => {
