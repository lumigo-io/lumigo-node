...
import { getEnvVarsMaskingRegex, getEventEntitySize, getHttpQueryParamsMaskingRegex, getRequestBodyMaskingRegex, getRequestHeadersMaskingRegex, getResponseBodyMaskingRegex, getResponseHeadersMaskingRegex, getSecretMaskingExactPath, getSecretPaths, isString, LUMIGO_SECRET_MASKING_ALL_MAGIC, LUMIGO_SECRET_MASKING_REGEX, LUMIGO_SECRET_MASKING_REGEX_BACKWARD_COMP, LUMIGO_WHITELIST_KEYS_REGEXES, OMITTING_KEYS_REGEXES, parseJsonFromEnvVar, safeExecute, BYPASS_MASKING_KEYS, isSecretMaskingDebug, } from '../utils';
...
const keyToRegexes = (regexesList = OMITTING_KEYS_REGEXES, backwardCompRegexEnvVarName = LUMIGO_SECRET_MASKING_REGEX_BACKWARD_COMP, regexesEnvVarName = LUMIGO_SECRET_MASKING_REGEX) => {
    ...
    const regexes = tryParseEnvVar(backwardCompRegexEnvVarName) || tryParseEnvVar(regexesEnvVarName) || regexesList;
...
const getItemsInPath = safeExecute((payload, path) => {
    ...
    return [];
}, 'Failed to find items to skip scrubbing', logger.LOG_LEVELS.WARNING, []);
...
function scrubPayloadBasedOnExactPath(originalPayload) {
    ...
    if (secretPaths.length > 0) {
        ...
        if (isString(payload)) {
            payload = safeExecute(scrubJsonStringBySecretPath, FAILED_SCRUBBING_BY_PATH, logger.LOG_LEVELS.DEBUG, payload)(payload, secretPaths, uniquePaths, '');
        }
        else {
            payload = safeExecute(scrubJsonBySecretPath, FAILED_SCRUBBING_BY_PATH, logger.LOG_LEVELS.DEBUG, payload)(payload, secretPaths, uniquePaths, '');
        }
    }
    return payload;
}
