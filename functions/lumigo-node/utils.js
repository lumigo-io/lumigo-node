...
export const LUMIGO_DEFAULT_DOMAIN_SCRUBBERS = '["secretsmanager.*.amazonaws.com", "ssm.*.amazonaws.com", "kms.*.amazonaws.com", "sts..*amazonaws.com"]';
...
export const LUMIGO_SECRET_MASKING_REGEX_HTTP_REQUEST_BODIES = 'LUMIGO_SECRET_MASKING_REGEX_HTTP_REQUEST_BODIES';
export const LUMIGO_SECRET_MASKING_REGEX_HTTP_REQUEST_HEADERS = 'LUMIGO_SECRET_MASKING_REGEX_HTTP_REQUEST_HEADERS';
export const LUMIGO_SECRET_MASKING_REGEX_HTTP_RESPONSE_BODIES = 'LUMIGO_SECRET_MASKING_REGEX_HTTP_RESPONSE_BODIES';
export const LUMIGO_SECRET_MASKING_REGEX_HTTP_RESPONSE_HEADERS = 'LUMIGO_SECRET_MASKING_REGEX_HTTP_RESPONSE_HEADERS';
export const LUMIGO_SECRET_MASKING_REGEX_HTTP_QUERY_PARAMS = 'LUMIGO_SECRET_MASKING_REGEX_HTTP_QUERY_PARAMS';
...
export const getRequestTimeout = () => {
    if (process.env[REQUEST_TIMEOUT_FLAG_MS])
        return parseInt(process.env[REQUEST_TIMEOUT_FLAG_MS]);
    return 300;
};
export const getContextInfo = (context) => {
    const remainingTimeInMillis = context.getRemainingTimeInMillis();
    const { functionName, awsRequestId, invokedFunctionArn, callbackWaitsForEmptyEventLoop } = context;
...
export const getAccountIdFromInvokedFunctinArn = (invokedFunctionArn) => invokedFunctionArn ? invokedFunctionArn.split(':')[4] : '';
...
export const splitXrayTraceIdToFields = (awsXAmznTraceId, maxFields = 100) => {
    const matches = Array.from(awsXAmznTraceId.matchAll(xRayFieldKeyValuePattern)).slice(0, maxFields);
    return Object.fromEntries(matches.map((match) => [match[1], match[2]]));
};
export const getNewFormatTraceId = (awsXAmznTraceId) => {
    const fields = splitXrayTraceIdToFields(awsXAmznTraceId);
    const root = fields.Root;
    if (!root) {
        throw new Error(`X-Ray trace ID is missing the Root field (_X_AMZN_TRACE_ID=${awsXAmznTraceId})`);
    }
    const rootValueSplit = root.split('-');
    if (rootValueSplit.length < 3) {
        throw new Error(`X-Ray trace ID Root field is not in the expected format (Root=${fields.Root}, _X_AMZN_TRACE_ID=${awsXAmznTraceId})`);
    }
...
export const getTraceId = (awsXAmznTraceId) => {
    try {
        logger.debug(`Parsing the _X_AMZN_TRACE_ID environment variable (_X_AMZN_TRACE_ID = ${awsXAmznTraceId})`);
        return getNewFormatTraceId(awsXAmznTraceId);
    }
    catch (e) {
        logger.warn(`Failed parsing the _X_AMZN_TRACE_ID environment variable, falling back to legacy parsing implementation (_X_AMZN_TRACE_ID = ${awsXAmznTraceId})`, e);
    }
    try {
        if (!awsXAmznTraceId) {
        ...
        if (traceIdArr.length < 3) {
            throw new Error('Expected 3 semi-colon separated parts in _X_AMZN_TRACE_ID environment variable.');
        }
        const traceId = {};
        ...
        if (!traceId['Root'] || !traceId['Sampled']) {
            throw new Error(`Either Root or Sampled tokens weren't found in the _X_AMZN_TRACE_ID environment variable.`);
        }
        if (!traceId['Parent']) {
         ...
         */
        logger.warn('Could not parse the value of the _X_AMZN_TRACE_ID environment variable (error is available in debug logs setting ' +
        ...
        if (!traceId) {
            ...
            logger.warn('Could find neither the _X_AMZN_TRACE_ID environment variable, nor the invocation identifier in the context; ' +
...
export const isPromise = (obj) => typeof (obj === null || obj === void 0 ? void 0 : obj.then) === 'function';
export const getAWSEnvironment = () => {
    const { AWS_REGION: awsRegion, _X_AMZN_TRACE_ID: awsXAmznTraceId, AWS_EXECUTION_ENV: awsExecutionEnv, LAMBDA_TASK_ROOT: awsLambdaTaskRoot, LAMBDA_RUNTIME_DIR: awsLambdaRuntimeDir, AWS_LAMBDA_FUNCTION_NAME: awsLambdaFunctionName, AWS_LAMBDA_LOG_GROUP_NAME: awsLambdaLogGroupName, AWS_LAMBDA_LOG_STREAM_NAME: awsLambdaLogStreamName, AWS_LAMBDA_FUNCTION_VERSION: awsLambdaFunctionVersion, AWS_LAMBDA_FUNCTION_MEMORY_SIZE: awsLambdaFunctionMemorySize, } = process.env;
    return {
        awsRegion,
        awsExecutionEnv,
        awsXAmznTraceId,
...
export const safeJsonParse = (obj, dflt = undefined) => {
    return safeExecute(() => JSON.parse(obj), 'Failed to parse json', logger.LOG_LEVELS.DEBUG, dflt)();
};
export const isTimeoutTimerEnabled = () => !validateEnvVar(TIMEOUT_ENABLE_FLAG, 'FALSE');
export const getTimeoutTimerBuffer = () => {
    if (process.env[TIMEOUT_BUFFER_FLAG_MS])
        return parseFloat(process.env[TIMEOUT_BUFFER_FLAG_MS]);
    if (process.env[TIMEOUT_BUFFER_FLAG])
        return parseFloat(process.env[TIMEOUT_BUFFER_FLAG]) * 1000;
    const { context } = TracerGlobals.getHandlerInputs();
    // @ts-ignore
    const { remainingTimeInMillis } = getContextInfo(context);
    return Math.max(500, Math.min(remainingTimeInMillis / 10, 3000));
};
export const getTracerMaxDurationTimeout = () => {
    if (process.env[TRACER_TIMEOUT_FLAG])
        return parseFloat(process.env[TRACER_TIMEOUT_FLAG]);
    const { context } = TracerGlobals.getHandlerInputs();
    if (isAwsContext(context)) {
        return Math.max(Math.min(TracerGlobals.getLambdaTimeout() / 5, MAX_TRACER_ADDED_DURATION_ALLOWED), MIN_TRACER_ADDED_DURATION_ALLOWED);
    }
    return MAX_TRACER_ADDED_DURATION_ALLOWED;
};
export const getAgentKeepAlive = () => {
    if (process.env[AGENT_KEEPALIVE])
        return parseFloat(process.env[AGENT_KEEPALIVE]);
    return undefined;
};
export const getTimeoutMinDuration = () => {
    if (process.env[TIMEOUT_MIN_DURATION])
        return parseFloat(process.env[TIMEOUT_MIN_DURATION]);
...
export const isProvisionConcurrencyInitialization = () => process.env.AWS_LAMBDA_INITIALIZATION_TYPE === 'provisioned-concurrency';
...
export const isSecretMaskingDebug = () => validateEnvVar(LUMIGO_SECRET_MASKING_DEBUG);
export const isLambdaWrapped = () => validateEnvVar(WRAPPED_FLAG);
export const shouldPropagateW3C = () => !validateEnvVar(LUMIGO_PROPAGATE_W3C, 'FALSE');
export const setLambdaWrapped = () => {
...
export const isSwitchedOff = () => safeExecute(() => {
    var _a;
    return validateEnvVar(SWITCH_OFF_FLAG) ||
        ((_a = TracerGlobals === null || TracerGlobals === void 0 ? void 0 : TracerGlobals.getTracerInputs()) === null || _a === void 0 ? void 0 : _a.switchOff) ||
        !isValidAlias();
})();
export const isStepFunction = () => validateEnvVar(IS_STEP_FUNCTION_FLAG) || TracerGlobals.getTracerInputs().isStepFunction;
export const getValidAliases = () => safeExecute(() => {
...
export const getMaxRequestSizeOnError = () => TracerGlobals.getTracerInputs().maxSizeForRequestOnError;
...
export const getInvokedAliasOrNull = () => safeExecute(() => {
...
export const isValidAlias = () => {
    ...
    const validAlias = !currentAlias || validAliases.length === 0 || validAliases.includes(currentAlias);
    if (!validAlias) {
...
export const lowerCaseObjectKeys = (o) => o ? Object.keys(o).reduce((c, k) => ((c[k.toLowerCase()] = o[k]), c), {}) : {};
export const isAwsService = (host, responseData = undefined) => {
...
export const addHeaders = (currentHeaders, headersToAssign) => Object.assign({}, currentHeaders, headersToAssign);
...
export const spanHasErrors = (span) => !!(span.error ||
    (span.info &&
        span.info.httpInfo &&
...
export const parseQueryParams = (queryParams) => {
    return safeExecute(() => {
        ...
        }, 'Failed to parse a specific key in parseQueryParams', logger.LOG_LEVELS.DEBUG));
        return obj;
    }, 'Failed to parse query params', logger.LOG_LEVELS.WARNING, {})();
};
const domainScrubbers = () => JSON.parse(process.env.LUMIGO_DOMAINS_SCRUBBER || LUMIGO_DEFAULT_DOMAIN_SCRUBBERS).map((x) => new RegExp(x, 'i'));
...
export function safeExecute(callback, message = 'Error in Lumigo tracer', logLevel = logger.LOG_LEVELS.WARNING, defaultReturn = undefined) {
...
export function safeExecuteAsync({ fn, message = 'Error in Lumigo tracer', logLevel = logger.LOG_LEVELS.WARNING, defaultReturn = undefined, }) {
    return function (...args) {
...
export const isEncodingType = (encodingType) => !!(encodingType &&
...
export const getAutoTagKeys = () => safeExecute(() => {
    return (process.env.LUMIGO_AUTO_TAG || DEFAULT_AUTO_TAG_KEY).split(',');
})() || [DEFAULT_AUTO_TAG_KEY];
export const filterObjectKeys = (obj, filterFunc) => Object.keys(obj)
    .filter(filterFunc)
    .reduce((cur, key) => {
 ...
 */
export const getMaxSizeForStoredSpansInMemory = () => {
...
export const getRequestBodyMaskingRegex = () => process.env[LUMIGO_SECRET_MASKING_REGEX_HTTP_REQUEST_BODIES];
export const getRequestHeadersMaskingRegex = () => process.env[LUMIGO_SECRET_MASKING_REGEX_HTTP_REQUEST_HEADERS];
export const getResponseBodyMaskingRegex = () => process.env[LUMIGO_SECRET_MASKING_REGEX_HTTP_RESPONSE_BODIES];
export const getResponseHeadersMaskingRegex = () => process.env[LUMIGO_SECRET_MASKING_REGEX_HTTP_RESPONSE_HEADERS];
export const getEnvVarsMaskingRegex = () => process.env[LUMIGO_SECRET_MASKING_REGEX_ENVIRONMENT];
export const getHttpQueryParamsMaskingRegex = () => process.env[LUMIGO_SECRET_MASKING_REGEX_HTTP_QUERY_PARAMS];
export const getSecretMaskingExactPath = () => process.env[LUMIGO_SECRET_MASKING_EXACT_PATH];
