...
const sqsParserJsonProtocol = (requestData, responseData) => {
    ...
    const messageId = safeGet(parsedResBody, ['MessageId'], undefined) ||
...
const sqsParserXmlProtocol = (requestData, responseData) => {
    ...
    const resourceName = parsedReqBody ? parsedReqBody['QueueUrl'] : undefined;
    // @ts-ignore
    const messageId = safeGet(parsedResBody, ['SendMessageResponse', 'SendMessageResult', 'MessageId'], undefined) ||
        safeGet(parsedResBody, [
...
export const sqsParser = (requestData, responseData) => {
    ...
    if (caseInsensitiveGet(reqHeaders, 'content-type', '').toLowerCase() ===
