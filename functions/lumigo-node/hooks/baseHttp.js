...
import { getCurrentTransactionId, getHttpInfo, getHttpSpan, getServiceData, } from '../spans/awsSpan';
import { addHeaders, getAWSEnvironment, getEdgeHost, getEventEntitySize, getPatchedTraceId, getRandomId, isAwsService, isEmptyString, isKeepHeadersOn, isTimeoutTimerEnabled, isValidAlias, lowerCaseObjectKeys, safeExecute, shouldPropagateW3C, } from '../utils';
...
import { extractBodyFromEmitSocketEvent, extractBodyFromWriteOrEndFunc, httpDataToString, } from './httpUtils';
import { URL } from 'url';
...
export class BaseHttp {
     ...
     */
    static onRequestCreated({ options, url, }) {
    ...
    static _getHostFromOptionsOrUrl({ options, url = undefined }) {
        if (url) {
     ...
     */
    static createRequestDataWriteHandler({ requestData, currentSpan = undefined, }) {
        return function (args) {
            ...
            if (isEmptyString(requestData.body)) {
                ...
                BaseHttp.aggregateRequestBodyToSpan(body, requestData, currentSpan, getEventEntitySize(true));
            }
     ...
     */
    static createRequestSocketDataWriteHandler({ requestData, currentSpan = undefined, }) {
    ...
    static createResponseDataWriterHandler({ transactionId, awsRequestId, requestData, requestRandomId, response, }) {
        ...
        return function (args) {
            ...
            if (args[0] === 'end') {
                ...
                const httpSpan = getHttpSpan(transactionId, awsRequestId, requestRandomId, requestData, Object.assign({ truncated }, responseData));
    ...
    static aggregateRequestBodyToSpan(body, requestData, currentSpan, maxSize = getEventEntitySize(true)) {
        let serviceData = {};
        if (body && !requestData.truncated && typeof body === 'string') {
            ...
            if (truncated)
                requestData.body = requestData.body.substr(0, maxSize);
            requestData.truncated = truncated;
        }
        if (currentSpan) {
            // @ts-ignore
            currentSpan.info.httpInfo = getHttpInfo(requestData, {});
            Object.assign(currentSpan.info, Object.assign({}, serviceData.awsServiceData));
        }
    }
    static parseHttpRequestOptions(options = {}, url) {
        const host = BaseHttp._getHostFromOptionsOrUrl({ options, url });
    ...
    static scrubQueryParams(search) {
        return (safeExecute(() => {
...
], BaseHttp, "_getHostFromOptionsOrUrl", null);
__decorate([
    GlobalDurationTimer.timedSync()
], BaseHttp, "parseHttpRequestOptions", null);
function isErroneousResponse(responseData) {
