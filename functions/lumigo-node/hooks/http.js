...
export class Http {
    ...
    static httpAfterRequestWrapper(args, originalFnResult, extenderContext) {
        ...
        const { requestData, requestRandomId, isTracedDisabled, awsRequestId, transactionId, currentSpan, } = extenderContext;
        if (isTracedDisabled) {
            return;
        }
        const endWrapper = BaseHttp.createRequestDataWriteHandler({ requestData, currentSpan });
        const emitWrapper = Http.httpRequestEmitBeforeHookWrapper(transactionId, awsRequestId, requestData, requestRandomId, currentSpan);
    ...
    static httpRequestEmitBeforeHookWrapper(transactionId, awsRequestId, requestData, requestRandomId, currentSpan) {
        const emitResponseHandler = Http.createEmitResponseHandler(transactionId, awsRequestId, requestData, requestRandomId);
...
], Http, "httpRequestArguments", null);
__decorate([
    GlobalDurationTimer.timedSync()
], Http, "httpBeforeRequestWrapper", null);
__decorate([
    GlobalDurationTimer.timedSync()
], Http, "httpAfterRequestWrapper", null);
__decorate([
    GlobalDurationTimer.timedSync()
], Http, "addStepFunctionEvent", null);
