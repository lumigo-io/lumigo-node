...
const getStaticProperties = (nodule) => {
    const baseProperties = ['prototype', 'name', 'length'];
    return Object.getOwnPropertyNames(nodule).filter((x) => !baseProperties.includes(x));
};
const injectMonitoringCommand = (args) => {
    switch (args.length) {
        case 0:
            logger.warn('MongoDB 4.x instrumentation skipped: missing expected arguments to MongoClient constructor');
            break;
        case 1:
            args.push({
            ...
            break;
        default:
            const optionsArgumentType = typeof args[1];
            switch (optionsArgumentType) {
                ...
                case 'undefined':
                    ...
                    break;
                default:
                    logger.warn(`MongoDB 4.x instrumentation skipped: unexpected type of the 'options' argument: ${optionsArgumentType}`);
            }
    }
    return args;
};
export const wrapMongoClient4xClass = (mongoClientLibrary) => {
    try {
        ...
        const wrapper = (originalFn) => {
            ...
            // client instance
            wrappedClass.connect = function (url, options, callback) {
