...
export class Http2 {
    static http2RequestArguments(args) {
        ...
        if (args[0] &&
            ...
            options = {
                method,
                headers: Object.assign({}, headers), // Copy all headers
                path,
                protocol: `${scheme}:`,
                hostname: authority,
                host: authority,
    ...
    static http2AfterConnectWrapper(args, originalFnResult) {
        ...
        extender.hook(originalFnResult, 'request', {
            ...
            afterHook: (args, stream, extenderContext) => {
                ...
                const { requestData, requestRandomId, awsRequestId, transactionId, currentSpan } = extenderContext;
                ...
                const emitWrapper = Http2.http2StreamEmitBeforeHookWrapper(transactionId, awsRequestId, requestData, requestRandomId, currentSpan);
    ...
    static http2StreamEmitBeforeHookWrapper(transactionId, awsRequestId, requestData, requestRandomId, currentSpan) {
        ...
        return function (args) {
            ...
            if (args[0] === 'response') {
                ...
                const formattedResponse = {
                    headers: Object.assign({}, responseObj), // Copy all headers from response
...
], Http2, "http2RequestArguments", null);
__decorate([
    GlobalDurationTimer.timedSync()
], Http2, "http2AfterConnectWrapper", null);
