...
import { getAutoTagKeys, getJSONBase64Size, getMaxSizeForStoredSpansInMemory, isLambdaTraced, spanHasErrors, } from './utils';
...
export const DEFAULT_MAX_SIZE_FOR_SPANS_STORED_IN_MEMORY = DEFAULT_MAX_SIZE_FOR_REQUEST_ON_ERROR * 10;
export const MAX_TRACER_ADDED_DURATION_ALLOWED = 750;
export const MIN_TRACER_ADDED_DURATION_ALLOWED = 200;
const warnSpansSizeOnce = runOneTimeWrapper((threshold, currentSize) => {
    logger.info(`Lumigo tracer is no longer collecting data on the invocation - maximum size of total spans collected (${threshold} bytes allowed, current size is ${currentSize} bytes)`);
}, {});
export var DroppedSpanReasons;
(function (DroppedSpanReasons) {
    ...
    DroppedSpanReasons["SPANS_STORED_IN_MEMORY_SIZE_LIMIT"] = "SPANS_STORED_IN_MEMORY_SIZE_LIMIT";
    ...
    DroppedSpanReasons["SPANS_SENT_SIZE_LIMIT"] = "SPANS_SENT_SIZE_LIMIT";
    // If the tracer added latency reached the maximum configured, we will stop collecting spans
    DroppedSpanReasons["INVOCATION_MAX_LATENCY_LIMIT"] = "INVOCATION_MAX_LATENCY_LIMIT";
})(DroppedSpanReasons || (DroppedSpanReasons = {}));
export class SpansContainer {
    static addSpan(span, ignoreSizeLimits = false) {
    ...
    static recordDroppedSpan(reason, incrementTotalSpansCounter = true, numOfDroppedSpans = 1) {
    ...
    static changeSpanId(oldId, newId) {
    ...
    static getTotalSpans() {
        return this.totalSpans;
    }
}
SpansContainer.spans = {};
SpansContainer.currentSpansSize = 0;
SpansContainer.totalSpans = 0;
SpansContainer.droppedSpansReasons = {};
...
export const ExecutionTags = (() => {
    ...
    const validateTag = (key, value, shouldLogErrors = true) => {
        ...
        if (key.length < 1 || key.length > MAX_TAG_KEY_LEN) {
            shouldLogErrors &&
                logger.warnClient(`${ADD_TAG_ERROR_MSG_PREFIX}: key length should be between 1 and ${MAX_TAG_KEY_LEN}: ${key} - ${value}`);
            return false;
        }
        if (value.length < 1 || value.length > MAX_TAG_VALUE_LEN) {
            shouldLogErrors &&
                logger.warnClient(`${ADD_TAG_ERROR_MSG_PREFIX}: value length should be between 1 and ${MAX_TAG_VALUE_LEN}: ${key} - ${value}`);
            return false;
        }
        // @ts-ignore
        if (global.tags.length >= MAX_TAGS) {
            shouldLogErrors &&
                logger.warnClient(`${ADD_TAG_ERROR_MSG_PREFIX}: maximum number of tags is ${MAX_TAGS}: ${key} - ${value}`);
            return false;
        }
        return true;
    };
    ...
    const getValue = (autoTagEvent, innerKey) => {
...
export const TracerGlobals = (() => {
    ...
    const setTracerInputs = ({ token = '', debug = false, edgeHost = '', switchOff = false, stepFunction = false, maxSizeForRequest = null, maxSizeForRequestOnError = null, lambdaTimeout = MAX_TRACER_ADDED_DURATION_ALLOWED, maxSizeForStoredSpansInMemory = DEFAULT_MAX_SIZE_FOR_SPANS_STORED_IN_MEMORY, }) => {
        const parsedMaxSizeForRequestOnError = maxSizeForRequestOnError ||
        ...
        const parsedMaxSizeForStoredSpansInMemory = maxSizeForStoredSpansInMemory || parsedMaxSizeForRequestOnError * 10;
        Object.assign(tracerInputs, {
    ...
    const clearTracerInputs = () => Object.assign(tracerInputs, {
        token: '',
        debug: false,
        edgeHost: '',
        switchOff: false,
